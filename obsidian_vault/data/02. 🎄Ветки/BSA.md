...
> WHERE branch = [[BSA]] AND project
> SORT date DESC, project ASC
> ``` 

> [!note] Прочие заметки по Business and Systems Analyst
> ```dataview
> TABLE WITHOUT ID
> 	file.link AS "Название",
> 	date AS "Опубликовано"
...
```dataviewjs
const group = "BSA - 1";
const projects = dv.pages('"data/99. 🗃️Картотека/Все проекты/BSA"')
    .filter(page => 
...
const projects = dv.pages('"data/99. 🗃️Картотека/Все проекты/BSA"').filter(page => 
    page.tags?.includes('project') && 
    !page.tags?.includes('index') && 
